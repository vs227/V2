...
@app.post("/autotrade/toggle", response_model=APIResponse, tags=["AutoTrade"])
def toggle_autotrade():
    try:
        autotrade = get_autotrade_service()
        if autotrade.is_enabled:
            autotrade.disable()
            msg = "AutoTrade disabled"
        else:
            autotrade.enable()
            msg = "AutoTrade enabled"
        return APIResponse(success=True, message=msg, data={"enabled": autotrade.is_enabled})
    except Exception as e:
        logger.error(f"AutoTrade toggle failed: {e}")
        return APIResponse(success=False, message=str(e))
    ...
    try:
        ...
        return APIResponse(
            ...
            data={
                ...
                "last_analysis": autotrade.last_analysis
            }
        )
    except Exception as e:
